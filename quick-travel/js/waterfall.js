$("document").ready(function(){
    ...
    var waterfallAdd = function(parent,imgData){
        $.each(imgData,function(index,data){
           ...
           // parent.append('<a href="http://localhost/showtourist/details.html?id='+imgData[index].id+'" class="list-group-item"><img class="lazy" width="420" data-original="'+imgData[index].imgs+'"/><ul><li>'+imgData[index].location +'</li><li>'+ imgData[index].tips+'</li></ul></a>'); 
    ...
    $('#waterfall').on('click','a',function(e){
        ...
        //window.open("http://simpletravel.eduren.net/details.html?location"+$(this).data(location));
        //$(this).href="http://simpletravel.eduren.net/details.html?location"+$(this).data(location);
    });
